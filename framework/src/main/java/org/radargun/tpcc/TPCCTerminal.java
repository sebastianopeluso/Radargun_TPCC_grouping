...
public class TPCCTerminal {
    ...
    public TPCCTerminal(int localWarehouse, long c_run, long c_id, long ol_i_id, boolean forceGlobalWrite){
    ...
    */
    ...
    public void executeTransaction(CacheWrapper cacheWrapper, int transaction, boolean isPassiveReplication, int indexNode, int indexThread, int numNodes, int numThreads)  throws Throwable
    {
    	long terminalWarehouseID;
    	if(this.localWarehouse==-1){
    		terminalWarehouseID=(TPCCTools.OVERLAP==0.0)?TPCCTools.randomNumber(1,TPCCPopulation.NB_WAREHOUSE):TPCCTools.generateId(1,TPCCPopulation.NB_WAREHOUSE,isPassiveReplication, indexNode, indexThread, numNodes, numThreads, TPCCTools.OVERLAP);
    	}
    	else{
    	
    		terminalWarehouseID=this.localWarehouse;
    	}
    	switch(transaction)
        {
            case NEW_ORDER:
                //long districtID = TPCCTools.randomNumber(1, 10);
                long districtID = (TPCCTools.OVERLAP==0.0)?TPCCTools.randomNumber(1, 10): TPCCTools.generateId(1,10,isPassiveReplication, indexNode, indexThread, numNodes, numThreads, TPCCTools.OVERLAP);
                long customerID = (TPCCTools.OVERLAP==0.0)?TPCCTools.nonUniformRandom(this.c_id, TPCCTools.A_C_ID, 1, TPCCTools.NB_MAX_CUSTOMER): TPCCTools.generateId( 1, TPCCTools.NB_MAX_CUSTOMER,isPassiveReplication, indexNode, indexThread, numNodes, numThreads, TPCCTools.OVERLAP);
                ...
                for(int i = 0; i < numItems; i++) // clause 2.4.1.5
                {
                    ...
                    itemIDs[i] = (TPCCTools.OVERLAP==0.0)?TPCCTools.nonUniformRandom(this.ol_i_id, TPCCTools.A_OL_I_ID, 1, TPCCTools.NB_MAX_ITEM):TPCCTools.generateId(1, TPCCTools.NB_MAX_ITEM, isPassiveReplication, indexNode, indexThread, numNodes, numThreads, TPCCTools.OVERLAP);
            ...
            case PAYMENT:
                ...
                districtID = (TPCCTools.OVERLAP==0.0)?TPCCTools.randomNumber(1, 10):TPCCTools.generateId(1, 10, isPassiveReplication, indexNode, indexThread, numNodes, numThreads, TPCCTools.OVERLAP);
                ...
                if(y <= 60)
                {
					...
					int indexForLastName = (int)((TPCCTools.OVERLAP==0.0)?TPCCTools.nonUniformRandom(this.c_run, TPCCTools.A_C_LAST, 0, TPCCTools.MAX_C_LAST):TPCCTools.generateId( 0, TPCCTools.MAX_C_LAST,isPassiveReplication, indexNode, indexThread, numNodes, numThreads, TPCCTools.OVERLAP));
					
                    customerLastName = lastName(indexForLastName);

                }
                else
                {
                    customerByName = false; 
                    customerID = (TPCCTools.OVERLAP==0.0)?TPCCTools.nonUniformRandom(this.c_id, TPCCTools.A_C_ID, 1, TPCCTools.NB_MAX_CUSTOMER): TPCCTools.generateId( 1, TPCCTools.NB_MAX_CUSTOMER,isPassiveReplication, indexNode, indexThread, numNodes, numThreads, TPCCTools.OVERLAP);
    ...
    private void paymentTransaction(CacheWrapper cacheWrapper, long w_id, long c_w_id, double h_amount, long d_id, long c_d_id, long c_id, String c_last, boolean c_by_name )  throws Throwable
    {
        ...
        if(c_by_name)
        {
            new_c_last=c_last;
            List cList=null;
            cList=CustomerDAC.loadByCLast2(cacheWrapper, c_w_id, c_d_id, new_c_last);
            while(cList==null || cList.isEmpty()){
                 ...
                 new_c_last=lastName((int)TPCCTools.nonUniformRandom(this.c_run, TPCCTools.A_C_LAST, 0, TPCCTools.MAX_C_LAST));
                  //new_c_last=lastName((int)TPCCTools.generateId(0, TPCCTools.MAX_C_LAST, isPassiveReplication, indexNode, indexThread, numNodes, numThreads, TPCCTools.OVERLAP));
                 cList=CustomerDAC.loadByCLast2(cacheWrapper, c_w_id, c_d_id, new_c_last);
        ...
        if(this.forceGlobalWrite){
        	cacheWrapper.put(null, "GLOBAL_KEY", new Integer(3));
        }
    ...
    private void orderStatusTransaction(CacheWrapper cacheWrapper, long w_id, long d_id, long c_id, String c_last, boolean c_by_name) throws Throwable{
        ...
        if(c_by_name)
        {
            List<Customer> cList=CustomerDAC.loadByCLast2(cacheWrapper, w_id, d_id, c_last);
